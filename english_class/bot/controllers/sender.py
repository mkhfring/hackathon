...
from english_class.constants.messages import LogMessage, ReplyMessage, \
    start_btns, InvoiceData, enter_btns, \
    main_menu_btns
from english_class.constants.states import BotState
from english_class.database.models import UserQuiz, Quiz, User
from english_class.database.quires import add_user, \
    get_all_not_sent_information, update_is_sent_status, \
    get_user_related_quiz, save_writing, get_user_related_info
...
def register(bot, update):
    ...
    bot.send_invoice(chat_id=update.message.chat_id, title=InvoiceData.title,
                     description=InvoiceData.description,
                     payload="payload",
                     provider_token=InvoiceConfig.provider_token,
                     start_parameter="", currency=InvoiceData.currency,
...
def get_location(bot, update):
    user = update.message.from_user
    bot.send_message(chat_id=user.id,
                     text="Dear *user* \nWe have to call this api to get "
                          "more *scores*:)\n"
                          "Sorry your location will not save in our "
                          "database.\nBye")
...
def take_quiz(bot, update):
    ...
    reply_keyboard = [[the_quiz.option_1, the_quiz.option_2, the_quiz.option_3,
                       the_quiz.option_4]]
    ...
    bot.send_message(chat_id=user.id, text=question,
                     reply_markup=ReplyKeyboardMarkup(keyboard=reply_keyboard))
    ...
    states={
        ...
        BotState.menu: [start_handler,
                        RegexHandler(pattern='^Enter to class 😋$',
                                     callback=main_menu)],
        BotState.selected_menu_option: [start_handler,
                                        RegexHandler(pattern='^show lesson$',
                                                     callback=show_information),
                                        RegexHandler(
                                            pattern='^send writing essay$',
                                            callback=get_writing),
                                        RegexHandler(pattern='^take a quiz$',
                                                     callback=take_quiz),
                                        ],
        BotState.next_information: [start_handler,
                                    RegexHandler(pattern='^main menu$',
                                                 callback=main_menu),
                                    RegexHandler(pattern='^next information',
                                                 callback=show_information)
                                    ],
        BotState.get_location: [start_handler,
                                MessageHandler(filters=Filters.location,
                                               callback=get_location)],
        BotState.save_writing: [MessageHandler(filters=Filters.document,
                                               callback=save_writing_file)],
        BotState.check_quiz: [start_handler,
                              MessageHandler(filters=Filters.text,
                                             callback=check_quiz)]
